...
class RedisAccountService(
    private val personRepository: Person1Repository,
...
//    private val redisTemplate: RedisTemplate<String, PersonEntity>,
    ...
    override fun events() = streamOperations.read(ChangeEvent::class.java, StreamOffset.fromStart(EVENTS))
//        .readWithType<String, ChangeEvent>(StreamOffset.fromStart(EVENTS))
    ...
    private suspend fun publishChangeEvent(prefix: String, id: String?, action: Action) {
        if (ceEnabled) {
            streamOperations.add(ObjectRecord.create(ecKeyspace, ChangeEvent("$prefix:$id", action = action)))
        }
    }
