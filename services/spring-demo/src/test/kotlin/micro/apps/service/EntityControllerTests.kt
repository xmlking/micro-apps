...
import micro.apps.model.fixtures.mockPersonDto
...
class EntityControllerTests(private val client: WebTestClient) : FunSpec() {
    ...
    init {
        ...
        test("test EntityController.entity").config(enabled = true, tags = setOf(Mock)) {
            // given
            val id = "www"
            coEvery { repository.get(id) } returns mockPersonDto(1)
            ...
            withClue("Fails with this clue") {
                response.expectStatus().isOk
                    .expectBody<Person>().isEqualTo(mockPersonDto(1))
            }
