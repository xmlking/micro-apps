...
```bash
gradle :pipelines:classifier:integrationTest --tests "micro.apps.pipeline.PubSubProducerTest.generateTestData"
```
...
gradle :pipelines:classifier:run --args="--runner=DirectRunner \
--project=${PROJECT_ID} \
--jobName=${PIPELINE_NAME} \
--pubsubRootUrl=${PUBSUB_EMULATOR_HOST}"

# or via jar [gradle :pipelines:classifier:build -x test]
java -jar ./pipelines/classifier/build/libs/classifier-0.1.2-SNAPSHOT-all.jar --runner=DirectRunner \
--project=${PROJECT_ID} \
--jobName=${PIPELINE_NAME} \
--pubsubRootUrl=${PUBSUB_EMULATOR_HOST}
```
...
gradle :pipelines:classifier:run --args="--runner=DataflowRunner \
--project=${PROJECT_ID} \
--jobName=${PIPELINE_NAME} \
...
# Or with fatJar
java -jar ./pipelines/classifier/build/libs/classifier-0.1.2-SNAPSHOT-all.jar --runner=DataflowRunner \
--project=${PROJECT_ID} \
--jobName=${PIPELINE_NAME} \
--gcsBucket=${GCS_BUCKET} \
--inputSubscription=projects/${PROJECT_ID}/subscriptions/${PIPELINE_NAME}-input \
...
```bash
gradle :pipelines:classifier:test
```
...
```bash
# clean
gradle :pipelines:classifier:clean
# make fatJar
gradle :pipelines:classifier:build -x test
```
